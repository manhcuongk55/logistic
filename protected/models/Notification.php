...
class Notification extends BaseNotification
{
	...
	public function getUrl(){
		switch($this->user_type){
			case self::USER_TYPE_CUSTOMER:
				...
				break;
			case self::USER_TYPE_COLLABORATOR:
				if($this->type==self::TYPE_ORDER_STATUS_CHANGED){
					$orderId = $this->getParam("order_id");
					return Yii::app()->controller->createUrl("/collaborator/orders",array(
						"order_id" => $orderId
					));
				} else if($this->type==self::TYPE_DELIVERY_ORDER_STATUS_CHANGED){
					$deliveryOrderId = $this->getParam("delivery_order_id");
					return Yii::app()->controller->createUrl("/collaborator/delivery_orders",array(
						"delivery_order_id" => $deliveryOrderId
					));
				}
				break;
	...
	public static function push($userType,$userId, $type, $dataParams=null){
