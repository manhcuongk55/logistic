<?php 
    if(!$item->model->real_exchange_rate){
        ...
        $profit = ($item->model->total_price_ndt + $item->model->total_delivery_price_ndt) * ($item->model->exchange_rate - $item->model->real_exchange_rate);
    }
    
    Util::controller()->data["totalProfit"] += $profit;

    $price = ($item->model->total_price_ndt + $item->model->total_delivery_price_ndt) * $item->model->exchange_rate;
